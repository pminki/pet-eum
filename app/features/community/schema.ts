import { bigint, integer, pgTable, primaryKey, text, timestamp, uuid, type AnyPgColumn } from "drizzle-orm/pg-core";
...
export const reviews = pgTable("reviews", {
  revies_id: bigint({ mode: "number" }),
  service_id: bigint({ mode: "number" }).notNull().references(() => services.service_id),
  reviewer_id: bigint({ mode: "number"} ).notNull(),
  rating: integer().notNull(),
  comment: text(),
  created_at: timestamp("created_at").defaultNow(),
},
  (table) => ({
    rating_position: sql`CHECK ($table.age} > 0)`,
  })
);