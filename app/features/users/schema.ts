import { bigint, jsonb, pgEnum, pgSchema, pgTable, text, timestamp, uuid, primaryKey, varchar, real, date } from "drizzle-orm/pg-core";
...
export const notifications = pgTable("notifications", {
  ...
  service_id: bigint({ mode: "number" }).references(() => services.service_id, {
    onDelete: "cascade",
  }),
...
export const pets = pgTable("pets", {
  pet_id: bigint({ mode: "number" }).primaryKey().generatedAlwaysAsIdentity(),
  profile_id: uuid().references(() => profiles.profile_id, {
    onDelete: "cascade",
  }),
  name: varchar({ length: 255 }).notNull(),
  species: varchar({ length: 50}),
